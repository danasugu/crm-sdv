        ...
        <div class="col-md-12">
            @can('sd_v_project_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.sd-v-projects.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $sdVProject->id }}">
                                        ...
                                        <td>
                                            @can('sd_v_project_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sd-v-projects.show', $sdVProject->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sd_v_project_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.sd-v-projects.edit', $sdVProject->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sd_v_project_delete')
                                                <form action="{{ route('frontend.sd-v-projects.destroy', $sdVProject->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sd_v_project_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sd-v-projects.massDestroy') }}",
